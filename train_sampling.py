...
def evaluate(model, g, nfeat, labels, val_nid, device):
    ...
    """
    model.eval()
    # print("val/test feat:", nfeat.shape)      # [169343,128]
    with th.no_grad():
...
def run(args, device, data):
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        # blocks.
        tic_step = time.time()
        print("train dataloader:", len(dataloader))     # 91
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            # Load the input features as well as output labels
            # print("blocks:", len(blocks), blocks[0].srcdata['feat'].shape)  # 3 [3w,128]
            batch_inputs, batch_labels = load_subtensor(train_nfeat, train_labels,
                                                        seeds, input_nodes, device)
            blocks = [block.int().to(device) for block in blocks]

            # Compute loss and prediction
            # print("train:", batch_inputs.shape)       [1.6w, 128]
        ...
        toc = time.time()
        print('Epoch {}  Time(s): {:.4f}'.format(epoch, toc - tic))
        if epoch >= 5:
            avg += toc - tic
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--num-hidden', type=int, default=256)   #16
    argparser.add_argument('--num-layers', type=int, default=3)
    argparser.add_argument('--fan-out', type=str, default='5,8,10') # 10,25
